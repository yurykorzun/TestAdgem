...
public class IronSourceEvents : MonoBehaviour
{
    ...
    private static event Action<IronSourceImpressionData> _onImpressionFailureEvent;

    public static event Action<IronSourceImpressionData> onImpressionFailureEvent
    {
        add
        {
            if (_onImpressionFailureEvent == null || !_onImpressionFailureEvent.GetInvocationList().Contains(value))
            {
                _onImpressionFailureEvent += value;
            }
        }

        remove
        {
            if (_onImpressionFailureEvent.GetInvocationList().Contains(value))
            {
                _onImpressionFailureEvent -= value;
            }
        }
    ...
    public void onImpressionFailure(string args)
    {
        if (_onImpressionFailureEvent != null)
        {
            IronSourceImpressionData impressionData = new IronSourceImpressionData(args);
            _onImpressionFailureEvent(impressionData);
        }
